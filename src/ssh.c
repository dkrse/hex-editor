...
gboolean ssh_path_is_remote(const char *path) {
    return (strstr(path, "/hex-editor-sftp-") != NULL);
}
...
const char *ssh_to_remote_path(const char *ssh_mount, const char *ssh_remote_path,
    ...
    size_t llen = strlen(local_path);

    /* Validate local_path starts with ssh_mount */
    if (llen < mlen || strncmp(local_path, ssh_mount, mlen) != 0) {
...
void ssh_ctl_start(char *ctl_dir, size_t ctl_dir_size,
    ...
    snprintf(ctl_dir, ctl_dir_size, "%s/hex-ssh-XXXXXX", runtime);
    if (!mkdtemp(ctl_dir)) {
        ctl_dir[0] = '\0';
        ctl_path[0] = '\0';
        return;
...
/* ── Remote file reading (binary-safe) ── */
...
/* ── Remote file writing (binary-safe) ── */
